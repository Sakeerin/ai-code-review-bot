  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Last {rows.length} GitHub PR and GitLab MR reviews</p>
      </div>
      ...
      {rows.length === 0 ? (
        <div className="border border-border border-dashed rounded-xl p-12 text-center">
          <p className="text-muted-foreground">
            {org
              ? "No reviews yet. Open a pull request or merge request in a connected repository to trigger your first review."
              : "Install the GitHub App or connect GitLab webhooks first to start reviewing."}
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-secondary/50">
              <tr>
                ...
                <th className="text-center px-4 py-3 font-medium text-muted-foreground">Platform</th>
            ...
            <tbody className="divide-y divide-border">
              {rows.map(({ review, repoFullName, repoUrl }) => (
                <tr key={review.id} className="hover:bg-secondary/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${PROVIDER_STYLES[review.provider] ?? "bg-secondary text-muted-foreground"}`}>
                      {review.provider}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                  ...
                  <td className="px-4 py-3 text-center tabular-nums">{review.commentsPosted}</td>
                  <td className="px-4 py-3 text-center">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${STATUS_STYLES[review.status] ?? "bg-secondary text-muted-foreground"}`}>
                      {review.status}
                    </span>
                  </td>
